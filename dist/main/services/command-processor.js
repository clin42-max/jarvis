...
exports.commandProcessor = exports.CommandProcessor = void 0;
...
class CommandProcessor {
    async process(text) {
        ...
        if ((0, encryption_1.isDestructiveCommand)(cmd)) {
        ...
        const usageCount = parseInt(settings_service_1.memoryService.getPreference(`cmd_count_${cmd.split(' ')[0]}`) ?? '0', 10);
        settings_service_1.memoryService.rememberPreference(`cmd_count_${cmd.split(' ')[0]}`, String(usageCount + 1));
        ...
        switch (action) {
            ...
            case 'open': {
                ...
                if (app) {
                    const launched = await system_service_1.systemService.launchApp(app);
                    return {
                        success: launched,
            ...
            case 'close': {
                ...
                if (app) {
                    const closed = await system_service_1.systemService.closeApp(app);
                    return {
                        success: closed,
            ...
            case 'search': {
                ...
                if (query) {
                    await system_service_1.systemService.searchWeb(query);
            ...
            case 'youtube': {
                ...
                if (query) {
                    await system_service_1.systemService.searchWeb(query, 'youtube');
            ...
            case 'folder': {
                ...
                for (const [key, folder] of Object.entries(folderMap)) {
                    if (cmd.includes(key)) {
                        await system_service_1.systemService.openFolder(folder);
            ...
            case 'create_folder': {
                ...
                if (name) {
                    const path = await system_service_1.systemService.createFolder(name);
                    return {
                        success: !!path,
            ...
            case 'hide':
                return { success: true, response: 'Going into background mode, sir.', action: 'hide', data: { mode: 'quick' } };
            case 'show':
                return { success: true, response: 'Restoring interface, sir.', action: 'show' };
            case 'stealth':
                return { success: true, response: 'Activating stealth mode, sir.', action: 'hide', data: { mode: 'stealth' } };
            case 'orb':
                return { success: true, response: 'Switching to orb mode, sir.', action: 'hide', data: { mode: 'orb' } };
            case 'widget':
                return { success: true, response: 'Activating widget mode, sir.', action: 'hide', data: { mode: 'widget' } };
            case 'calculate': {
                try {
            ...
            case 'define': {
                ...
                if (word) {
                    await system_service_1.systemService.searchWeb(`define ${word}`);
                    return { success: true, response: `Looking up the definition of ${word}, sir.`, action: 'define' };
                }
                break;
            }
            case 'system_stats': {
                const stats = await system_service_1.systemService.getStats(true);
                return {
                    success: true,
            ...
            default: {
                ...
                const response = await ai_service_1.aiService.chat(text, taskType);
                return { success: true, response, action: 'ai_chat', data: { taskType } };
            }
        }
        const response = await ai_service_1.aiService.chat(text);
    ...
    extractAfter(cmd, prefixes) {
        for (const prefix of prefixes) {
            const idx = cmd.indexOf(prefix);
            if (idx !== -1)
                return cmd.slice(idx + prefix.length).trim();
        }
        return null;
    }
    inferTaskType(cmd) {
        if (/code|program|debug|function|script|api/.test(cmd))
            return 'coding';
        if (/search|research|find|compare|article|read/.test(cmd))
            return 'research';
        if (/automate|workflow|schedule|when|every/.test(cmd))
            return 'automation';
        if (/why|explain|analyze|think|reason/.test(cmd))
            return 'reasoning';
        return 'general';
    }
}
exports.CommandProcessor = CommandProcessor;
exports.commandProcessor = new CommandProcessor();
